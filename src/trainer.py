# imported AdamW from torch.optim instead of transformers
import json
import os
import random
import numpy as np
...
class RoSTERTrainer(object):
    ...
    def __init__(self, args):
        ...
        if args.do_train:
            ...
            self.gce_bin_weight = torch.ones_like(all_input_ids).to(self.device).float()
            self.gce_type_weight = torch.ones_like(all_input_ids).to(self.device).float()
    ...
    def noise_robust_train(self, model_idx=0):
        if os.path.exists(os.path.join(self.temp_dir, f"y_pred_{model_idx}.pt")) and not self.sweep:
            ...
            early_stopper = EarlyStopping(tolerance=3, min_delta=0.05)
